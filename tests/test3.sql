...
BEGIN
    ...
    INSERT INTO umbox_image(name, file_name) VALUES ('U2 HTTP Auth Proxy', 'u2-http-auth-proxy.qcow2') RETURNING id INTO u7;
    INSERT INTO umbox_image(name, file_name) VALUES ('u14 DLC Max Login Detection', 'u14-dlc-max-login.qcow2') RETURNING id INTO u6;
    INSERT INTO umbox_image(name, file_name) VALUES ('u4-block-all', 'u4-block-all.qcow2') RETURNING id INTO u4;
    INSERT INTO umbox_lookup(state_id, device_type_id, umbox_image_id, dag_order) VALUES (normal.id, dtype.id, u7.id, 1);
    INSERT INTO umbox_lookup(state_id, device_type_id, umbox_image_id, dag_order) VALUES (suspicious.id, dtype.id, u6.id, 1);
